module Moderator
  class AuditController < ApplicationController
    ...
    def index
      ...
      records = records.where('created_at >= ?', params[:date_from].to_date) if params[:date_from].present?
      records = records.where('created_at <= ?', params[:date_to].to_date.end_of_day) if params[:date_to].present?
